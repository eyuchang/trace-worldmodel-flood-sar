...
from trace_jepa.scenario.delta.validation_v7 import (
    _book_and_performance,
    _cluster_mean_interval,
    _trace_artifacts_verify,
    run_v7_study,
)
...
def _paired_reconciliation(
    *,
    ...
    for metric in metric_names:
        ...
        metrics[metric] = {
            "baseline": _cluster_mean_interval(
                baseline_values, protocol_hash, f"{study_id}:baseline:{metric}"
            ),
            "selected": _cluster_mean_interval(
                selected_values, protocol_hash, f"{study_id}:selected:{metric}"
            ),
            "paired_difference_selected_minus_baseline": _cluster_mean_interval(
...
def run_v8_registered_validation(
    *,
    ...
    book_gates = {
        ...
        "trace_and_outcome_links_verified": _trace_artifacts_verify(book_result),
    }
    confirmatory_gates = {
