...
CASE_MANIFEST_PATH = Path(__file__).resolve().with_name("cases.json")


def _find_trace_repo() -> Path:
    """Locate the pinned editable TRACE checkout prepared by the instructor."""

    starts: list[Path] = []
    configured = os.environ.get("TRACE_SMALL_SAR_BASE")
    if configured:
        starts.append(Path(configured).expanduser().resolve())
    starts.extend(
        (
            Path(trace_jepa.__file__).resolve(),
    ...
    for start in starts:
        for candidate in (start, *start.parents):
            if (
                (candidate / "configs/scenarios/wf_dfld_01_small.yaml").is_file()
                and (candidate / "data/scenario/delta/reference").is_dir()
            ):
                return candidate
    raise RuntimeError(
        "The prepared TRACE Small runtime was not found. "
        "Ask an instructor to open the workshop environment."
    )


REPO_ROOT = _find_trace_repo()
...
def _controller_event_for_case(
    spec: Mapping[str, Any],
    decisions: Sequence[Mapping[str, Any]],
) -> Mapping[str, Any]:
    """Select the one public controller event named by a case specification."""

    return _one(
        decisions,
        label=f"controller event {spec['call_id']}",
        predicate=lambda item: item.get("call_id") == spec.get("call_id")
        and item.get("trace_record_id") == spec.get("trace_record_id")
        and item.get("trace_record_version") == spec.get("trace_record_version"),
    )
...
def _validate_evidence_link(
    ...
    """Confirm that the selected TRACE record cites the selected visible evidence."""

    call_id = str(spec["call_id"])
    evidence_raw = _one(
        evidence,
        label=f"evidence {call_id}",
        predicate=lambda item: item.get("evidence_id") == f"evidence-{call_id}",
    )
    try:
        validated = WorldModelEvidence.model_validate(evidence_raw)
    except ValidationError as exc:
    ...
    if validated.evidence_id not in selected_record["evidence_refs"]:
        raise LabDataError("TRACE record does not reference the selected public evidence")


def _commitment_and_outcome(
    event: Mapping[str, Any],
    authorization: TraceAuthorization,
    commitments: Sequence[Mapping[str, Any]],
    outcomes: Sequence[Mapping[str, Any]],
) -> tuple[Mapping[str, Any] | None, Mapping[str, Any] | None]:
    """Validate and return the optional commitment/outcome pair for one event."""
    ...
    commitment_raw = _one(
        ...
        predicate=lambda item: item.get("commitment_id") == commitment_id,
    )
    try:
    ...
    if (
        commitment.authorizing_record_id != authorization.record_id
        or commitment.authorizing_record_version != authorization.record_version
    ):
        raise LabDataError("commitment is authorized by a different TRACE record version")

    outcome = _one(
        outcomes,
        label=f"outcome for {commitment_id}",
        predicate=lambda item: item.get("authorizing_commitment_id") == commitment_id,
    )
    if (
        outcome.get("authorizing_trace_record_id") != authorization.record_id
        or outcome.get("authorizing_trace_record_version") != authorization.record_version
    ):
        raise LabDataError("outcome points to a different TRACE record version")
    return commitment_raw, outcome
...
def _build_case(
    ...
    """Project one validated public decision chain into the small lab types."""

    event = _controller_event_for_case(spec, artifacts["controller_decisions.json"])
    request, resources = _request_for_call(
        str(spec["call_id"]),
        str(event["belief_cluster_id"]),
        artifacts["calls.json"],
        artifacts["predictor_requests.json"],
    )
    authorization = _authorization_for_case(spec, event, artifacts["trace_records.json"])
    _validate_evidence_link(
        spec,
        artifacts["evidence_ledger.json"],
        artifacts["trace_records.json"],
    )
    commitment, outcome = _commitment_and_outcome(
        event,
        authorization,
        artifacts["commitments.json"],
        artifacts["outcomes.json"],
    )
    return PublicCase(
...
def build_cases() -> dict[str, PublicCase]:
    ...
    return {
        str(spec["case_id"]): _build_case(spec, artifacts)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--controller",
        choices=("book", "exercise", "solution"),
        default="exercise",
